...
/**
 * Decode persisted settings. v1 settings migrate to v2 by gaining the
 * default encoding; anything else unknown is rejected.
 */
export function decodeNovelSettings(raw: string): NovelSettings | undefined {
	try {
		const parsed = JSON.parse(raw) as Record<string, unknown>
		if (parsed.v === 1) {
			// v1 settings gain the default encoding; the other fields
			// carried over unchanged.
			return { ...parsed, v: 2, encoding: "auto" } as NovelSettings
		}
		if (parsed.v !== SETTINGS_VERSION) return undefined
		return parsed as NovelSettings
	} catch {
		return undefined
	}
}
